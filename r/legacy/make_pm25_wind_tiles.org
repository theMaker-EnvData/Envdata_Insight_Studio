...
# --- 하루(9장) 전체 공통 컬러바 상한 계산 ---
pm25_max_all <- -Inf
for (tk_max in times_kst) {
  f <- cams_path(tk_max)
  if (!file.exists(f)) next
  r_cams_max <- rast(f)                   # 6 bands
  pm25_ug_max <- r_cams_max[[1]] * 1e9    # kg/m3 -> ug/m3
  tmpl_cams_max <- rast(ext(r_cams_max), resolution = GRID_DEG_CAMS, crs = crs(r_cams_max))
  r_pm25_krg_max <- terra::resample(pm25_ug_max, tmpl_cams_max, method = "bilinear")
  mx <- terra::global(r_pm25_krg_max, "max", na.rm = TRUE)[[1]]
  if (is.finite(mx)) pm25_max_all <- max(pm25_max_all, mx, na.rm = TRUE)
}
# 보호: 입력이 일부/전부 없을 때 기본값
...
# -----------------------------
plot_tile <- function(r_pm25_ug, r_u, r_v, tk, outfile_png) {
  bb <- ext(r_pm25_ug)
  # 바람 샘플 포인트 (표시 밀도 조절)
  nx <- 20; ny <- 15
  ...
  zlim <- range(values(r_pm25_ug), na.rm = TRUE)
  ...
  # 색상바
  plot(r_pm25_ug, legend.only = TRUE, col = pal(64), zlim = zlim, axes = TRUE,
...
for (i in seq_along(times_kst)) {
  ...
  plot_tile(r_pm25_krg, r_u_krg, r_v_krg, tk, out_png)
